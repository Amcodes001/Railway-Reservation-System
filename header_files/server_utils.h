...
#define TRAIN "./databases/train"
#define BOOKING "./databases/booking"
#define ERR_EXIT(msg) do{perror(msg);exit(EXIT_FAILURE);} while(0)

char *ACCOUNT[3] = {"./databases/customer", "./databases/agent", "./databases/admin"};
...
int login(int sock){
	...
	// Read the type of login : 1. Customer 2. Agent 3. Admin 4. Back
	...
	if((fd = open(ACCOUNT[type-1], O_RDWR | O_CREAT))==-1){
		printf("File Error, please check the file path in the code.\n");
		return 0;
	}
	...
	lock.l_whence = SEEK_SET; // determines where the l_start starts from .
	lock.l_pid = getpid();
	...
	if(type == 1){
		...
		if(temp.id == acc_no && !strcmp(temp.pass, password)){
			// Tell client that the login is valid.
			write(sock, &valid, sizeof(valid));

			// Now client will give the choice , which we will catch in menu1() of server.
			while(menu1(sock, temp.id, type) != -1);
...
int signup(int sock){
	...
	if(type == 4){ return 0;}	//when back is chosen.
...
int display_menu_admin(int sock, int id){
	...
	if(op_id == 1)
	{
		...
		int fd = open(TRAIN, O_RDWR | O_CREAT);
		struct flock lock;
		lock.l_type = F_WRLCK;
		lock.l_start = 0;
		lock.l_len = 0;
	...
	if(op_id == 5)
	{
		...
		int fd = open(ACCOUNT[type - 1], O_RDWR | O_CREAT);
		struct flock lock;
		lock.l_type = F_WRLCK;
		lock.l_start = 0;
		lock.l_whence = SEEK_SET;
...
int menu1(int sock, int id, int type){
	...
	if(op_id == 1){
		...
		// fp has the total number of bytes in the file, so iterate until lssk returns that.
		while(fp != lseek(fd, 0, SEEK_CUR)){
			read(fd, &temp, sizeof(struct train));
			av_trains[t_cnt++] = temp;

			// Sent the train info to the client.
			write(sock, &temp.tid, sizeof(int));
			write(sock, &temp.train_no, sizeof(int));	
		...
		int trainid, seats;
		// Now fetch the trainID from the client, for which the customer wants to book a seat.
		...
		for(int i=0; i<t_cnt; i++){
			if(av_trains[i].tid+1 == trainid){
				...
				// If it's a deleted train , can't book it.
		...
		// Send if the train is valid or not to the client. (Train is invalid if it's deleted or the trainID given by customer does not match with any of trainID in the DataBase).
		write(sock,&valid,sizeof(valid));
		...
		if(valid)
		{
			...
			if(seats>0){
				...
				int fd2 = open(BOOKING, O_RDWR | O_CREAT);
	...
	if(op_id == 3)
	{
		...
		int fd1 = open(TRAIN, O_RDWR | O_CREAT);
		int fd2 = open(BOOKING, O_RDWR | O_CREAT);
		...
		{//increase
			...
			if(temp2.seat_end - temp2.seat_start + 1 < val){
				printf("You have not booked enough seats.");

				op_id = -2;
				write(sock, &op_id, sizeof(op_id));
			}
			else if(temp2.seat_end - temp2.seat_start + 1 == val){
				temp2.cancelled=1;
				temp2.seat_end -= val;
				temp1.av_seats += val;
			}
	...
	if(op_id == 4) {
		...
		int fd = open(BOOKING, O_RDWR | O_CREAT);
		...
		if(booking.cancelled == 0 && booking.acc_no == id && booking.type == type){
			...
			booking.cancelled = 1;
			int fd2 = open(TRAIN,O_RDWR | O_CREAT);
...
void view_booking(int sock, int id, int type){
	int fd = open(BOOKING, O_RDONLY | O_CREAT);
	...
	if(fp == 0){// No bookings, then return 0 to client side.
