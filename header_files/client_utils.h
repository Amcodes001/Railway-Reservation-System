...
#define USER_CUSTOMER "./databases/customer"
#define USER_ADMIN "./databases/admin"
#define USER_AGENT "./databases/agent"
...
int entry_point(int sock){
	...
	if(choice_==1){
		...
		printf("Enter the type of account:(Write the corresponding number)\n");
		printf("1.Customer\n2.Agent\n3.Admin\n4.Back\n");
		...
		if(type == 4)
		{
			...
			// Return anything other than 3 , so that the main() function can call the entry_point() function again.
			return 0;
		}
		...
		//getpass() function keeps the written passwrod hidden.
		strcpy(password,getpass("Enter password: "));

		// Send everything to server to validate.
		write(sock, &type, sizeof(type));
...
int display_admin(int sock, int choice_){
	...
	switch(choice_)
	{
		...
		case 4:
		{
	        ...
	        {
	        case 1:
	            fd = open( USER_CUSTOMER,O_RDONLY | O_CREAT );
	            ...
	            break;
	        case 2:
	            fd = open(USER_AGENT,O_RDONLY | O_CREAT);
	            fp = lseek(fd,0,SEEK_SET);
	            while(read(fd,&acc,sizeof(acc)))
	                printf("%d\t\t%s\n",acc.id,acc.name);
	            close(fd);
	            break;
	        case 3:
	            fd = open(USER_ADMIN,O_RDONLY | O_CREAT);
	            fp = lseek(fd,0,SEEK_SET);
	            while(read(fd,&acc,sizeof(acc)))
	                printf("%d\t\t%s\n",acc.id,acc.name);
	            close(fd);
		...
		case 7:
		{
			//search users
			printf("\nhey 7\n");
			fd = open(BOOKING,O_RDONLY | O_CREAT);
        	fp = lseek(fd,0,SEEK_SET);
		...
		case 8:
		{
			fd = open(TRAIN,O_RDONLY | O_CREAT);
        	fp = lseek(fd,0,SEEK_SET);
        	struct train tr;
...
int display_users(int sock, int choice_){
	...
	switch(choice_){
		case 1:{
			...
			// The server returns the number of trains.
			...
			while(trains--){
				...
				// If this train is not deleted by the admin .
			...
			// If it's a deleted train , can't book it. So server will return valid=0. If train is deleted by admin.
			read(sock,&valid,sizeof(valid));

			if(valid){
				// Server will return the number of available seats.
				read(sock, &trainavseats, sizeof(trainavseats));
...
void view_booking(int sock)
{
	...
	while(entries--)
	{
		...
		if(!cancelled)
			printf("BookingID: %d\t1st Ticket: %d\tLast Ticket: %d\tNumber of Seats: %d\tTrainNo. :%d\n", bid+1, bks_seat, bke_seat, bke_seat-bks_seat+1 , tno);
	}
